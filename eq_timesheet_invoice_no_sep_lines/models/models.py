...
class AccountAnalyticLine(models.Model):
    ...
    @api.model
    def _get_to_invoice_default(self):
        if 'active_id' in self._context:
            active_id = self._context['active_id']
            project_obj = self.env['project.project'].search([('id','=',active_id)])
            return project_obj.eq_to_invoice_id.id
    ...
    to_invoice = fields.Many2one('timesheet.invoice.factor', 'Invoiceable',
                                 help="It allows to set the discount while making invoice, keep empty if the activities should not be invoiced.", default=_get_to_invoice_default)

    product_id = fields.Many2one(default=_default_product, domain="[('type', '=', 'service')]")
    ...
    def _get_invoice_price(self, account, product_id, user_id, qty):
        ...
        if account.partner_id.property_product_pricelist:
            price = account.partner_id.property_product_pricelist.price_get(product_id, qty or 1.0, account.partner_id.id)[account.partner_id.property_product_pricelist.id]
        else:
            price = 0.0
    ...
    def _prepare_cost_invoice_line(self, product_id, uom, user_id,
        ...
        if product_id:
            ...
            for line in analytic_lines:
                ...
                if data.date:
                    ...
                    #     lang_objs = lang_pool.search([('code', '=', self._context['lang'])])
                    #     if len(lang_objs) >= 1:
                    #         date_format = lang_objs[0].date_format
                    # date = time.strftime(date_format, time.strptime(line.date, '%Y-%m-%d'))
                    #details.append(date)
                    curr_invoice_line['eq_date'] = line.date
                if line.time_start:
                    start = str(datetime.timedelta(hours=line.time_start)).rsplit(':', 1)[0]
                    details.append(start)
                if line.time_stop:
                    stop = str(datetime.timedelta(hours=line.time_stop)).rsplit(':', 1)[0]
                    details.append(stop)
                if data.time:
                    if line.product_uom_id:
                        details.append("%s %s" % (str(datetime.timedelta(hours=line.unit_amount)).rsplit(':', 1)[0], line.product_uom_id.name))
                    else:
                        details.append("%s" % (str(datetime.timedelta(hours=line.unit_amount)).rsplit(':', 1)[0],))
                if data.name:
        ...
        for (key_id, company_id, currency_id), analytic_lines in invoice_grouping.items():
            ...
            for (product_id, uom, user_id, factor_id,
                ...
                curr_invoice_line = self.with_context(invoice_context)._prepare_cost_invoice_line(product_id, uom, user_id, factor_id, account,
                                                                    ...
                                                                    #journal_type
                                                                    data)
                exist_account_invoice = self.env['account.invoice'].search([('partner_id','=',partner.id),('state','=','draft')])
...
class TimesheetInvoiceCreate(models.TransientModel):
    ...
    time = fields.Boolean('Time spent', help='The time of each work done will be displayed on the invoice', default=True)
    name = fields.Boolean('Description', help='The detail of each work done will be displayed on the invoice', default=True)
    price = fields.Boolean('Cost', help='The cost of each work done will be displayed on the invoice. You probably don\'t want to check this', default=True)
...
class eq_project_configuration(models.TransientModel):
    _inherit = 'project.config.settings'

    default_project_product = fields.Many2one('product.product',string='Default product for service [timesheet_invoice]', domain="[('type', '=', 'service')]")


    def set_default_values(self):
        ir_values_obj = self.env['ir.values']

        ir_values_obj.set_default('project.task', 'default_project_product', self.default_project_product.id or False)


    def get_default_values(self, fields):
        ir_values_obj = self.env['ir.values']

        default_product = ir_values_obj.get_default('project.task', 'default_project_product')

        return {
            'default_project_product': default_product,
        }


class eq_project_project(models.Model):
    _inherit = "project.project"

    @api.model
    def _get_default_to_invoice(self):
        timesheet_invoice_factor_objs = self.env['timesheet.invoice.factor'].search([('factor','=',0)])
        if len(timesheet_invoice_factor_objs) > 1:
            return timesheet_invoice_factor_objs[0].id
        else:
            return timesheet_invoice_factor_objs.id

    eq_to_invoice_id = fields.Many2one('timesheet.invoice.factor', string='Invoice Rate', default=_get_default_to_invoice)
