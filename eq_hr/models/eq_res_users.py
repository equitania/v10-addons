...
class eq_hr_res_users(models.Model):
    ...
    def create(self, values):
        ...
        ##### Überprüft, ob Flag in den Grundeinstellungen gesetzt ist und somit ein
        ##### Versenden einer Reset-Passwort Email stattfinden soll
        ...
        if 'eq_employee_id' in values and 'do_not_repeat' not in self._context:
            if values['eq_employee_id']:
                ...
                # Sets the user_id in the employee. do_not_repeat in context so that
                # the employee does not set the employee_id for the user.
