        ...
        if new_records:
            ...
            for cur_file in new_files:
                try:
                    ...
                    file_name = "".join(
                        [x if x.isalnum() or x in [' ', '-', '+', '.'] else "_" for x in file_name])
                    ...
                    self.create_log(
                        ...
                        self.module_info,
                        "EXCEPTION",
                        "Error '" + str(ex) + "' while copying file to " + file_target,
                        self.use_log)
    def _get_param_value_as_bool(self, param_name):
        """
        Hilfsmethode für Auslesen von Parameterwerten aus der Tabelle
        ir.config_parameter mit Umwandlung in einen boolean
        :param param_name:
        :return:
        """
